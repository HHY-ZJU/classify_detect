...
def imgclassify (img):
    # 图片预处理设置
    transformer = caffe.io.Transformer({'data': net0.blobs['data'].data.shape})  # 设定图片的shape格式(1,3,28,28)
    transformer.set_transpose('data', (2, 0, 1))  # 改变维度的顺序，由原始图片(28,28,3)变为(3,28,28)
    ...
    net0.blobs['data'].data[...] = transformer.preprocess('data', image)  # 执行上面设置的图片预处理操作，并将图片载入到blob中
    ...
    prob = net0.blobs['softmax'].data[0].flatten()  # 取出最后一层（Softmax）属于某个类别的概率值，并打印
    print (prob)
    order = prob.argsort()[1]  # 将概率值排序，取出最大值所在的序号
    print( 'the class is:', labels[order] ) # 将该序号转换成对应的类别名称，并打印
    return labels[order]
...
#加载model
    ...
    root0='/home/yangyh/caffe_root/caffe/img_detect_try/'
    deploy0=root0+'deploy1.prototxt'
    caffe_model0=root0+'snapshot_iter_990.caffemodel'
    labels_filename = root0 + 'labels.txt'  # 类别名称文件，将数字标签转换回类别名称
    mean_file = root0 + 'mean.npy'
    net0 = caffe.Net(deploy0, caffe_model0, caffe.TEST)
    ...
    videoWriter = cv2.VideoWriter('saveVideo.avi',cv2.cv.CV_FOURCC('M','P','4','2'),24,(1920,1080))#最后一个是保存图片的尺寸
    for each in videos:
        frame = cv2.imread(os.path.join(img_root,each))
